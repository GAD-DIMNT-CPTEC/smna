...
module Mod_FLUXCO2Clima  ! SSTClima + CO2
  ...
  integer :: xMax, yMax, xmx, yMaxHf, mEnd1, mEnd2, mnwv0, mnwv1, mnwv2, mnwv3
...
contains
  ...
  subroutine initFLUXCO2Clima(nameListFileUnit, varCommon_)
    ...
    call getxMaxyMax (varCommon%mEnd, xMax, yMax)
    write (nLats(3:7), '(I5.5)') yMax
  ...
  function generateFLUXCO2Clima() result(isExecOk)
    ...
    isExecOk = .false.
    write (unit = p_nfprt, fmt = '(/,A,I6)') '   Imax = ', xMax
    write (unit = p_nfprt, fmt = '(A,I6,/)') '   Jmax = ', yMax
    call createSpectralRep (mEnd1, mEnd2, mnwv1)
    call createGaussRep (yMax, yMaxHf)
    call createFFT (xMax)
    call createLegTrans (mnwv0, mnwv1, mnwv2, mnwv3, mEnd1, mEnd2, yMaxHf)
    if (var%linear) then
      call initLinearInterpolation (var%xDim, var%yDim, xMax, yMax, lat0, lon0)
    else
      call initAreaInterpolation (var%xDim, var%yDim, xMax, yMax, flagInput, flagOutput)
    end if

    allocate (landSeaMask(xMax, yMax), seaIceMask(xMax, yMax))
    allocate (coefTopIn(mnwv2))
    allocate (coefTop(mnwv2), topog(xMax, yMax))
    allocate (co2In(var%xDim, var%yDim), sstIn(var%xDim, var%yDim), wrIn(var%xDim, var%yDim), co2Gaus(xMax, yMax), sstGaus(xMax, yMax), wrOut(xMax, yMax))
    allocate (seaIceFlagIn(var%xDim, var%yDim), seaIceFlagOut(xMax, yMax))
    ...
    call specCoef2Grid (mnwv2, mnwv3, mEnd1, mEnd2, xMax, yMax, xmx, yMaxHf, coefTop, topog)
    ...
    if (varCommon%grads) then
      ...
      write (unit = nfctl, fmt = '(A,I5,A,F8.3,F15.10)') &
        'XDEF ', xMax, ' LINEAR ', 0.0_p_r8, 360.0_p_r8 / real(xMax, p_r8)
      write (unit = nfctl, fmt = '(A,I5,A)') 'YDEF ', yMax, ' LEVELS '
      if (var%linear) then
        write (unit = nfctl, fmt = '(8F10.5)') gLatsL(yMax:1:-1)
      else
        write (unit = nfctl, fmt = '(8F10.5)') gLatsA(yMax:1:-1)
      end if

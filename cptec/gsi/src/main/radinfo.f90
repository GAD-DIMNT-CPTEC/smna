...
!
! abstract:  This routine reads the satinfo, satbias\_angle.in, and
!            satbias files.  
!
...
!
!            The satbias\_angle.in file contains the angle dependent part
...
!            instrument.
!
!            The satbias.in file contains the coefficients for the
...
!   Open file to bias file (satang=satbias_angle.in).  Read data.
    ...
    if (.not. adp_anglebc) then
       open(lunin,file='satbias_angle.in',form='formatted',status='old',iostat=istat)
       if (istat /= 0 ) then
          write(6,*)'RADINFO_READ:  ***ERROR*** file "satbias_angle.in" does not exist'
          call stop2(79)
       endif
       ...
       if (istat /= 0 ) then
          write(6,*)'RADINFO_READ:  ***ERROR*** file "satbias_angle.in" is empty'
          call stop2(79)
       endif
       ...
       if (istat /= 0 .OR. maxscan <= 0 .OR. maxscan > 1000) then
          write(6,*)'RADINFO_READ:  ***ERROR*** error reading satbias_angle.in, maxscan out of range: ',maxscan
          call stop2(79)
       endif
       read2: do
...
!          if (istat /= 0) exit
!!         some compilers cannot handle with this reading
!!          read(lunin,'(100(4x,10f7.3/))',iostat=istat,end=1111) &
!!               (cbiasx(ip),ip=1,maxscan)
!          do i=1,9      ! fixing the reading problem
!             k=(i-1)*10
!             read(lunin,'(4x,10f7.3)',iostat=istat,end=1111) &
!                  (cbiasx(ip),ip=1+k,10+k)
!             if (istat /= 0) exit
!          end do
!          read(lunin,'(a0)',iostat=istat,end=1111) ! fixing
!          if (istat /= 0) exit 
          read(lunin,'(A)',iostat=istat,end=1111) line
          if (istat /= 0) exit 
          read(line,'(I5,1x,A20,2x,I4,e15.6)') ich,isis,ichan,tlapm
          i = 0
          readlines: do while(trim(line) .ne. BLK)
             read(lunin,'(A)',iostat=istat,end=1111) line
             if (istat /= 0) exit 
             if (trim(line) .ne. BLK) &
                read(line,'(4x,90f7.3)') (cbiasx(i+ip),ip=1,10)
             i = i + 10
          end do readlines
          cfound = .false.
          do j =1,jpch_rad
          ...
          if(.not. cfound .and. mype == 0) &
               write(6,*) '***WARNING instrument/channel ',isis,ichan, &
               'found in satbias_angle.in file but not found in satinfo'
       end do read2
1111   continue
       close(lunin)
       if (istat>0) then
          write(6,*)'RADINFO_READ:  ***ERROR*** error reading satbias_angle.in, istat=',istat
          write(6,*)'RADINFO_READ:  stop program execution'
          call stop2(79)
       endif

       if (mype==mype_rad) then
          write(iout_rad,*)'RADINFO_READ:  read satbias_angle.in file'
          do j=1,jpch_rad
             if(.not. nfound(j))write(iout_rad,*) 'RADINFO_READ: ***WARNING instrument/channel ',&
                  nusis(j),nuchan(j),' not found in satbias_angle.in file - set to zero '
          end do
       end if
...
!   bias correction.  Open unit to input file.  Read data.
       ...
       open(lunin,file='satbias.in' ,form='formatted')
       nfound = .false.
       read4: do
          if (.not. adp_anglebc) then
          ...
          if(mype == 0 .and. .not. cfound) &
             write(6,*) '***WARNING instrument/channel ',isis,ichan, &
             'found in satbias.in file but not found in satinfo'
       end do read4
1333   continue
       close(lunin)
       if (istat>0) then
          write(6,*)'RADINFO_READ:  ***ERROR*** error reading satbias.in, istat=',istat
          write(6,*)'RADINFO_READ:  stop program execution'
          call stop2(79)
       endif
       ...
       if (mype==mype_rad) then
          ...
          do j=1,jpch_rad
             if (nfound(j)) then
                ...
                nusis(j),nuchan(j),' not found in satbias.in file - set to zero '
             endif
          end do
...
!         check inew_rad again
          ...
          if (mype==mype_rad) then
             open(lunout,file='satbias_angle.out',form='formatted')
...
!   Open unit to output file.  Write updated coefficients.  Close unit.
    open(lunout,file='satbias.out',form='formatted')
    rewind lunout
    if (.not. adp_anglebc) then
       do jch=1,jpch_rad

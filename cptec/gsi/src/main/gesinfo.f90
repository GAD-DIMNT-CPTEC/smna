...
!$$$
  ...
  use gridmod, only: idvc5,ak5,bk5,ck5,tref5,&
      ...
      ntracer,ncloud,nlat,nlon,idvm5,idlsm,&
  ...
  use constants, only: zero,h300,r60,r3600,i_missing,r0_01
...
! Declare local variables
  ...
  integer       :: nvcoord
  integer       :: ierr
  integer(i_kind),dimension(4):: itime
  integer(i_kind),dimension(4):: ftime
  ...
  character(len=64), parameter :: myname_= 'GESINFO'
...
110        format(3x,'k,ak,bk,ck,tref=',i3,1x,4(g19.12,1x))
  ...
  case ('CPTEC')
     ...
     ! Handle non-GMAO interface (ie, CPTEC interface)

     if (regional)then ! BRAMS
        ...
        inquire(file=fileDir,exist=fexist)
        if(.not.fexist) then
           write(6,*)' GESINFO:  GUESS FILE NOT AVAILABLE: PROGRAM STOPS ', trim(fileDir)
           call stop2(99)
           stop
        end if
        ...
        if (ierr.ne.0)then
           write(6,'(4(A,1x),I4)')trim(myname_),'Problem to open/read BAM file: ',trim(fileDir),':: status:',ierr
           call stop2(99)
        endif
        ...
        !
        !       Check for consistency:  levs, latb, lonb, jcap
        !

!        if (     IMax .ne.  nlon  .or. &
!             (JMax+2) .ne.  nlat  .or. & !two points to interpolations
!                 KMax .ne.  nsig  .or. &
!                 Mend .ne. jcap_b      &
!           )then
        ...
        if (     KMax .ne.  nsig  .or. &
           ...
           write (6,'(2(1x,A))') trim(myname_),': *** ERROR *** '
           write (6,'(1x,A,4(1x,I5))')'BAMHead (IMax, JMax+2, KMax,   Mend ) = ', IMax, JMax+2, KMax,   Mend
           write (6,'(1x,A,4(1x,I5))')'Namelist(nlon,   nlat, nsig, jcap_b ) = ', nlon,   nlat, nsig, jcap_b
           call stop2(99)
        ...
        idate4(1) = itime(4) ! hour
        idate4(2) = itime(2) ! month
        idate4(3) = itime(3) ! day
        idate4(4) = itime(1) ! year
        ...
        select case (nvcoord)
           ...
           case default
              ...
              write(6,*)'GESINFO:  ***ERROR*** INVALID value for nvcoord=',nvcoord
              call stop2(85)
...
!        ! >>>>>>>>  Necessary only for GFS <<<<<<<<<<
!        !
!        !  Load surface pressure and thermodynamic variable ids
!
!        idvm5   = gfshead%idvm
!
!        ! surface pressure identifier
!        !  0/1: ln(ps)
!        !    2: ps
!
!        idpsfc5 = mod ( gfshead%idvm,10 )
!
!        ! thermodynamic variable identifier
...
!        end do
        ...
        if(mype==mype_out) then
           write(6,120) Mend,KMax,JMax,IMax,&
                        ntracer,ncloud,nvcoord,&
                        idvm5,idsl5,idpsfc5,idthrm5
120        format('GESINFO:  jcap_b=',i4,', levs=',i3,', latb=',i5,&
                ', lonb=',i5,', ntrac=',i3,', ncloud=',i3,&
                ', nvcoord=',i3,', idvm=',i3,', idsl=',i3)
...
130        format(3x,'k,ak,bk,ck,tref=',i3,1x,4(g19.12,1x))
  ...
  case default
     if (mype==mype_out) &
          write(6,*)'GESINFO:  ***ERROR*** INVALID center name :',trim(center)
  ...
  select case (trim(center))
     ...
     case('CPTEC')
        ...
        if (regional)then ! BRAMS
           ...
           call read_brams_files(mype)

        else ! BAM

           call read_bamfiles(mype)

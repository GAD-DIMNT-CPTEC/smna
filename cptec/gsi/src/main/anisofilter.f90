...
! Allocatable variable must not be declared as a namelist-group-object!
  real(r_kind) :: afact(100) !< hardwired to can be used at namelist
...
! Declare local variables
  ...
  namelist/parmcardanisof/latdepend,scalex1,scalex2,scalex3,afact,hsteep, &
...
!---
     ...
     if (fexist) then
        ...
        close(55)
        !----------------------------------------------------------------------------------------------!
        ! This is to bypass a bug: Allocatable variable must not be declared as a namelist-group-object!

        afact0(1:nvars) = afact(1:nvars)

...
use crtm_module, only: crtm_atmosphere_type,crtm_surface_type,crtm_geometry_type, &
    ...
    crtm_kind => fp
use gridmod, only: lat2,lon2,nsig,msig,nvege_type,regional,wrf_mass_regional,netcdf,use_gfs_ozone, idlsm
use mpeu_util, only: die
use crtm_aod_module, only: crtm_aod_k

use gridmod, only: center_name
...
!  Note other module variables are only used within this routine
  ...
  integer(i_kind), parameter :: SAND                     =  3
  integer(i_kind), parameter :: ROCK                     =  4
...
!
! Mapping land vegetation cover types of BAM model to CRTM
!

  
  !
  ! BAM GENERIC VARIABLES TO INTERFACE WITH CRTM
  !
  integer(i_kind) :: BAM_VEG_N_TYPES
  integer(i_kind) :: BAM_SOI_N_TYPES
  integer(i_kind), allocatable :: BAM_VEG_TO_CRTM(:)
  integer(i_kind), allocatable :: BAM_LND_TO_CRTM(:)
  integer(i_kind), allocatable :: BAM_SOI_TO_CRTM(:)

  !---------------------------------!
  ! NPOESS Classification Scheme
  !---------------------------------!
  !Index: Surface Type Name
  !---------------------------------!
  !  1  : compacted soil
  !  2  : tilled soil
  !  3  : sand
  !  4  : rock
  !  5  : irrigated low vegetation
  !  6  : meadow grass
  !  7  : scrub
  !  8  : broadleaf forest
  !  9  : pine forest
  ...
  ! 18  : wet soil
  ! 19  : scrub soil
  ! 20  : broadlead70 pine30
  !---------------------------------!

  !
  ! BAM SSIB VARIABLES TO INTERFACE WITH CRTM
  !

  integer(i_kind), parameter :: BAM_SSIB_VEG_N_TYPES = 13
  integer(i_kind), parameter, dimension(0:BAM_SSIB_VEG_N_TYPES) :: bam_ssib_lnd_to_crtm = (/ &
                   COMPACTED_SOIL,         & !  0: water
                   BROADLEAF_FOREST,       & !  1: broadleaf-evergreen (tropical forest)
                   BROADLEAF_FOREST,       & !  2: broad-deciduous trees
                   ...
                   BROADLEAF_BRUSH,        & !  6: broadleaf trees with ground cover (savanna)
                   SCRUB,                  & !  7: ground cover only (perennial)
                   SCRUB,                  & !  8: broad leaf shrubs w/ ground cover
                   SCRUB_SOIL,             & !  9: broadleaf shrubs with bare soil
                   TUNDRA,                 & ! 10: dwarf trees & shrubs w/ground cover (tundra)
                   COMPACTED_SOIL,         & ! 11: bare soil
                   TILLED_SOIL,            & ! 12: crops
                   COMPACTED_SOIL          & ! 13: permanent ice

                 /)

  integer(i_kind), parameter :: BAM_IBIS_VEG_N_TYPES = 15
  integer(i_kind), parameter, dimension(0:BAM_IBIS_VEG_N_TYPES) :: bam_ibis_lnd_to_crtm = [ &
                   COMPACTED_SOIL,       & ! 0: Ocean /Lakes/Rivers
                   BROADLEAF_FOREST,     & ! 1: tropical evergreen
                   BROADLEAF_FOREST,     & ! 2: tropical deciduous
                   PINE_FOREST,          & ! 3: temperate evergreen broadleaf
                   PINE_FOREST,          & ! 4: temperate evergreen conifer
                   PINE_FOREST,          & ! 5: temperate deciduous forest
                   BROADLEAF_PINE_FOREST,& ! 6: boreal evergreen
                   BROADLEAF_PINE_FOREST,& ! 7: boreal deciduous
                   BROADLEAF_PINE_FOREST,& ! 8: mixed forest/woodland
                   BROADLEAF_BRUSH,      & ! 9: Savana
                   SCRUB,                & !10: grassland/steppe
                   SCRUB,                & !11: dense shrubland
                   SCRUB_SOIL,           & !12: open shrubland
                   TUNDRA,               & !13: Tundra
                   COMPACTED_SOIL,       & !14: Desert
                   COMPACTED_SOIL        & !15: polar desert/rock/ice          
                   ]
  !----------------------------------------------------------!
  ...
  !----------------------------------------------------------!
  !  0   : water
  !  1   : broadleaf-evergreen (tropical forest)
  !  2   : broad-deciduous trees
  !  3   : broadleaf and needleleaf trees (mixed forest)
  !  4   : needleleaf-evergreen trees
  !  5   : needleleaf-deciduous trees (larch)
  !  6   : broadleaf trees with ground cover (savanna)
  !  7   : ground cover only (perennial)
  !  8   : broad leaf shrubs w/ ground cover
  !  9   : broadleaf shrubs with bare soil
  ! 10   : dwarf trees & shrubs w/ground cover (tundra)
  ! 11   : bare soil
  ! 12   : crops
  ! 13   : permanent ice   
  !----------------------------------------------------------!

  integer(i_kind), parameter, dimension(0:BAM_SSIB_VEG_N_TYPES) :: bam_ssib_veg_to_crtm = (/ &
                                        0,& !  0: water
                                        1,& !  1: broadleaf-evergreen (tropical forest)
                                        2,& !  2: broad-deciduous trees
                                        3,& !  3: broadleaf and needleleaf trees (mixed forest)
                                        4,& !  4: needleleaf-evergreen trees
                                        5,& !  5: needleleaf-deciduous trees (larch)
                                        6,& !  6: broadleaf trees with ground cover (savanna)
                                        7,& !  7: ground cover only (perennial)
                                        8,& !  8: broad leaf shrubs w/ ground cover
                                        9,& !  9: broadleaf shrubs with bare soil
                                       10,& ! 10: dwarf trees & shrubs w/ground cover (tundra)
                                       11,& ! 11: bare soil
                                       12,& ! 12: crops
                                       13 & ! 13: permanent ice                                   
                 /)
                 
  integer(i_kind), parameter, dimension(0:BAM_IBIS_VEG_N_TYPES) :: bam_ibis_veg_to_crtm = [ &
                                        0, & ! 0: Ocean /Lakes/Rivers
                                        1, & ! 1: tropical evergreen
                                        2, & ! 2: tropical deciduous
                                        3, & ! 3: temperate evergreen broadleaf
                                        4, & ! 4: temperate evergreen conifer
                                        4, & ! 5: temperate deciduous forest
                                        5, & ! 6: boreal evergreen
                                        5, & ! 7: boreal deciduous
                                        3, & ! 8: mixed forest/woodland
                                        6, & ! 9: Savana
                                        7, & !10: grassland/steppe
                                        8, & !11: dense shrubland
                                        9, & !12: open shrubland
                                       10, & !13: Tundra
                                       11, & !14: Desert
                                       13  & !15: polar desert/rock/ice
                   ]
  ...
  integer(i_kind), parameter :: BAM_SSIB_SOI_N_TYPES = 12
  integer(i_kind), parameter, dimension(1:BAM_SSIB_SOI_N_TYPES) :: bam_ssib_soi_to_crtm = (/ &
                                                                 1, &! 1
                                                                 1, &! 2
                                                                 4, &! 3
                                                                 3, &! 4
                                                                 ...
                                                                 3  &!12
                                                                 /)
  integer(i_kind), parameter :: BAM_IBIS_SOI_N_TYPES = 12
  integer(i_kind), parameter, dimension(1:BAM_IBIS_SOI_N_TYPES) :: bam_ibis_soi_to_crtm = [ &
                                                                 1, & !  1: sand
                                                                 1, & !  2: Loamy Sand
                                                                 4, & !  3: Sand Loam
                                                                 6, & !  4: Loam
...
!         'crtm' in the naming convention.  
  integer(i_kind), parameter, dimension(0:13) :: gfs_to_crtm=(/&
                       COMPACTED_SOIL,         &!  0: water
                       BROADLEAF_FOREST,       &!  1: broadleaf-evergreen (tropical forest)
                       BROADLEAF_FOREST,       &!  2: broad-deciduous trees
                       BROADLEAF_PINE_FOREST,  &!  3: broadleaf and needleleaf trees (mixed forest)
                       ...
                       TUNDRA,                 &! 10: dwarf trees & shrubs w/ground cover (tundra)
                       COMPACTED_SOIL,         &! 11: bare soil
                       TILLED_SOIL,            &! 12: cultivations
                       COMPACTED_SOIL          &! 13: glacial
                       /)
! Mapping nmm to CRTM
  ...
  integer(i_kind), parameter, dimension(1:IGBP_N_TYPES) :: igbp_to_npoess=(/&
                       ...
                       BROADLEAF_PINE_FOREST,  &!  5: mixed forests
                       ...
                       INVALID_LAND,           &! 15: snow and ice (empty)
                       COMPACTED_SOIL,         &! 16: barren or sparsely vegetated
                       INVALID_LAND,           &! 17: water (empty)
                       TUNDRA,                 &! 18: wooded tundra
                       TUNDRA,                 &! 19: mixed tundra
                       TUNDRA                  &! 20: bare ground tundra
                       /)
 ...
 ! are based on a 16 category representation of FAO/STATSGO. 
  ...
  !
  ! BRAMS LEAF/JULES VARIABLES TO INTERFACE WITH CRTM
  !

  integer(i_kind), parameter :: BRAMS_VEG_N_TYPES = 21
  integer(i_kind), parameter, dimension(0:BRAMS_VEG_N_TYPES) :: brams_lnd_to_crtm = (/ & ! convert to NPOESS
                                        0,&! 0: Ocean
                                        0,&! 1: Lakes, rivers, streams
                                        0,&! 2: Ice cap/glacier
                                        0,&! 3: Desert, bare soil
                                        9,&! 4: Evergreen needleleaf tree
                                        9,&! 5: Deciduous needleleaf tree
                                        8,&! 6: Deciduous broadleaf tree
                                        8,&! 7: Evergreen broadleaf tree
                                        6,&! 8: Short grass
                                        6,&! 9: Tall grass
                                       10,&!10: Semi-desert
                                       10,&!11: Tundra
                                        7,&!12: Evergreen shrub
                                        7,&!13: Deciduous shrub
                                       12,&!14: Mixed woodland
                                        6,&!15: Crop/mixed farming, C3 grassland
                                        5,&!16: Irrigated crop
                                       18,&!17: Bog or marsh
                                       12,&!18: Wooded grassland
                                       15,&!19: Urban and built up
                                       18,&!20: Wetland evergreen broadleaf tree
                                       15 &!21: Very urban
                                      /)
  integer(i_kind), parameter, dimension(0:BRAMS_VEG_N_TYPES) :: brams_veg_to_crtm = (/ &
                                        0,&!  0: Ocean
                                        0,&!  1: Lakes, rivers, streams
                                       13,&!  2: Ice cap/glacier
                                       11,&!  3: Desert, bare soil
                                        4,&!  4: Evergreen needleleaf tree
                                        5,&!  5: Deciduous needleleaf tree
                                        2,&!  6: Deciduous broadleaf tree
                                        1,&!  7: Evergreen broadleaf tree
                                        7,&!  8: Short grass
                                        7,&!  9: Tall grass
                                        9,&! 10: Semi-desert
                                       10,&! 11: Tundra
                                        4,&! 12: Evergreen shrub
                                       10,&! 13: Deciduous shrub
                                        3,&! 14: Mixed woodland
                                       12,&! 15: Crop/mixed farming, C3 grassland
                                       12,&! 16: Irrigated crop
                                        2,&! 17: Bog or marsh
                                        3,&! 18: Wooded grassland
                                       11,&! 19: Urban and built up
                                        2,&! 20: Wetland evergreen broadleaf tree
                                       11 &! 21: Very urban
                                    /)

  integer(i_kind), parameter :: BRAMS_SOI_N_TYPES = 12
  integer(i_kind), parameter, dimension(1:BRAMS_SOI_N_TYPES) :: brams_soi_to_crtm = (/ &
                                        1,&! 1: sand
                                        1,&! 2: loamy sand
                                        0,&! 3: sandy loam
                                        5,&! 4: silt loam
...
!! RB-1.1.rev1855 CRTM
 ...
 atmosphere(1)%absorber_units(1) = MASS_MIXING_RATIO_UNITS   !(g/kg)
 atmosphere(1)%absorber_units(2) = VOLUME_MIXING_RATIO_UNITS !(ppmv)
 ...
 case ('CPTEC')
   select case (idlsm)
      case (1) ! SSiB
         ...
         BAM_VEG_N_TYPES = BAM_SSIB_VEG_N_TYPES
         BAM_SOI_N_TYPES = BAM_SSIB_SOI_N_TYPES

         allocate(bam_lnd_to_crtm(0:BAM_VEG_N_TYPES))
         bam_lnd_to_crtm      = bam_ssib_lnd_to_crtm

         allocate(bam_veg_to_crtm(0:BAM_VEG_N_TYPES))
         bam_veg_to_crtm      = bam_ssib_veg_to_crtm

         allocate(bam_soi_to_crtm(BAM_SOI_N_TYPES))
         bam_soi_to_crtm      = bam_ssib_soi_to_crtm

      case (3) ! IBIS

         BAM_VEG_N_TYPES = BAM_IBIS_VEG_N_TYPES
         BAM_SOI_N_TYPES = BAM_IBIS_SOI_N_TYPES

         allocate(bam_lnd_to_crtm(0:BAM_VEG_N_TYPES))
         bam_lnd_to_crtm      = bam_ibis_lnd_to_crtm

         allocate(bam_veg_to_crtm(0:BAM_VEG_N_TYPES))
         bam_veg_to_crtm      = bam_ibis_veg_to_crtm

         allocate(bam_soi_to_crtm(BAM_SOI_N_TYPES))
         bam_soi_to_crtm      = bam_ibis_soi_to_crtm


      case default
         write(6,*)myname_,': ***ERROR*** invalid land surface model from BAM Model.' &
           //' for BAM model you just can set SSiB (schemes=01) or IBIS(schemes=03) model.',&
           ' schemes =',idlsm, &
             '  ***STOP IN SETUPRAD***'
          call stop2(72)
   end select
...
!$$$
  ...
  if(regional.and.center.eq.'NCEP')deallocate(nmm_to_crtm_ir)
  if(regional.and.center.eq.'NCEP')deallocate(nmm_to_crtm_mwave)

  if(allocated(BAM_LND_TO_CRTM)) deallocate(BAM_LND_TO_CRTM)
  if(allocated(BAM_VEG_TO_CRTM)) deallocate(BAM_VEG_TO_CRTM)
  if(allocated(BAM_SOI_TO_CRTM)) deallocate(BAM_SOI_TO_CRTM)
...
!          GFS and NNM as of September 2005
     itype  = nint(data_s(ivty)) ! Tipo de vegetacao do arquivo externo de superficie (temporario no ponto na observacao)
     istype = nint(data_s(isty)) ! Tipo de solo do arquivo externo de superficie (temporario no ponto na observacao)
     select case (trim(center))
     case ('NCEP')
        if (regional) then
     ...
     case('CPTEC')
        if(regional)then
           itype  = min(max(0,itype),brams_veg_n_types)
           istype = min(max(1,istype),brams_soi_n_types)
           surface(1)%land_type       = max(1,brams_lnd_to_crtm(itype))
           surface(1)%vegetation_type = max(1,brams_veg_to_crtm(itype))
           surface(1)%soil_type       = brams_soi_to_crtm(istype)
           lai_type                   = itype ! <- verificar
        else
           itype  = min(max(0,itype),BAM_VEG_N_TYPES)
           istype = min(max(1,istype),BAM_SOI_N_TYPES)
           surface(1)%land_type       = bam_lnd_to_crtm(itype)
           surface(1)%Vegetation_Type = max(1,bam_veg_to_crtm(itype))
           surface(1)%Soil_Type       = bam_soi_to_crtm(istype)
           lai_type                   = itype
        endif
     end select
...
!
     ...
     if (trim(center).eq.'CPTEC'.and. regional)then
        if (brams_lnd_to_crtm(itype) .eq. 0)then
...
!  atmosphere(1)%level_pressure(0) = 0.0 ! r10*prsi_rtm(msig+1)
  do k = 1,msig
...
! Load profiles into extended RTM model layers
     ...
     if(lcf4crtm) then
        ...
        atmosphere(1)%absorber(k,1)  = r1000*q(kk2)*c3(kk2) ! Mix ratio (kg/kg)
     endif
     if(iozs==0) then
...
!     do k=0,msig
!        print*,'pressure:', mype, k, atmosphere(1)%level_pressure(k), log(atmosphere(1)%level_pressure(k))
!     enddo

...
! set passed variables to public
  ...
  public :: bramsPrslAvg, bramsPsfcAvg, RTGT, DZT
  ...
  logical IsBRAMS           ! .t. to run with BRAMS regional model
  ...
  character(10) center_name ! Name of center that come from First Guess
...
!                                        1: Philips
!                                        2: average
  integer(i_kind) idlsm             ! Land surface model Scheme (only BAM model)
!                                        1: SSib
!                                        2: Sib2
!                                        3: Ibis
...
! additional variables for regional mode
  ...
  real(r_kind),allocatable:: bramsPrslAvg(:)      ! Mean atmospheric profile
  real(r_kind)            :: bramsPsfcAvg         ! Mean surface pressure
  real(r_kind),allocatable:: RTGT(:,:)
  real(r_single),allocatable:: DZT(:)
...
!-------------------------------------------------------------------------
    ...
    center_name          = 'NCEP'    ! Name of center that come from First Guess
    nsig                 = 42        ! no. of vertical atmospheric levels
    nsig_soil            = 6         ! no. of levels of soil model
    nsig1o               = 7         ! max no. of levels distributed on each processor
    nlat                 = 96        ! no. of latitudes
    nlon                 = 384       ! no. of longitudes
    idvc5                = 1         ! vertical coordinate identifier
    idvm5                = 0
    idpsfc5              = 1         ! surface pressure identifier : 0/1 - ln(ps), 2 - ps
    idthrm5              = 1         ! thermodynamic variable identifier : 0/1 - Virt. Temp., 2 - sens. temp, 3 - enthalpy (CpT)
    idsl5                = 1         ! midlayer pressure definition : 1 - Philips, 2 - average
    idlsm                = 1         ! land surface model scheme: [BAM model: 1 - SSiB, 3 - IBIS]
    ntracer              = 1         ! number of tracers
    ncloud               = 0         ! number of cloud types
    gencode              = 80
    regional             = .false.   ! .t. for regional background/analysis
    periodic             = .false.   ! logical flag for periodic e/w domains
    IsBRAMS              = .false.   ! .t. to run with BRAMS model
    wrf_nmm_regional     = .false.   ! .t. to run with WRF NMMB model
    wrf_mass_regional    = .false.   ! .t. to run with WRF MASS model
    cmaq_regional        = .false.   ! .t. to run with CMAQ model
    nems_nmmb_regional   = .false.   ! .t. to run with NEMS NMMB model
    twodvar_regional     = .false.   ! .t. to run code in regional 2D-var mode
    use_gfs_ozone        = .false.   ! .t. to use gfs ozone in regional analysis
    check_gfs_ozone_date = .false.   ! .t. to date check gfs ozone against regional
    regional_ozone       = .false.   ! .t. to turn on ozone for regional analysis
    netcdf               = .false.   ! .t. for regional netcdf i/o
    filled_grid          = .false.   
    half_grid            = .false.
    grid_ratio_nmmb      = sqrt(two) ! ratio of analysis grid to nmmb model grid in nmmb model grid units.
    grid_ratio_wrfmass   = 1.0_r_kind! ratio of analysis grid to wrf model grid in wrf mass grid units.
    nmmb_reference_grid  = 'H'       ! nmmb grid type as reference for analysis grid [H or V]
    nmmb_verttype        = 'OLD'     ! vertical coordinate definition 
                                     !    OLD: p = eta1*pdtop+eta2*(psfc-pdtop-ptop)+ptop]
                                     !    NEW: p = eta1*pdtop+eta2*(psfc-ptop)+ptop
    lat1                 = nlat      ! no. of lats on subdomain (no buffer)
    lon1                 = nlon      ! no. of lons on subdomain (no buffer)
    lat2                 = lat1+2    ! no. of lats on subdomain (buffer points on ends)
    lon2                 = lon1+2    ! no. of lons on subdomain (buffer points on ends)
...
! !USES:
    ...
    use constants, only: zero, one, three, deg2rad,rad2deg, pi,half, two,r0_01, cp, cpor, p00
...
!-------------------------------------------------------------------------
    ...
    real(r_single),allocatable:: dx(:,:),dy(:,:)
    ...
    character(len=256), parameter :: myname_ = 'init_reg_glob_ll :: '
...
! This is global run
    ...
    if (IsBRAMS)then
       ...
       if(diagnostic_reg.and.mype==0) then
          WhatPrint = 'Initializating for BRAMS regional run'
          write(6,'(2(1x,A))')trim(myname_),trim(WhatPrint)
       endif

! Get regional grid informations

       ihr=-999
       do i=0,12
          write(FileHeader,'("BRAMS.head.",I2.2)') i
          inquire(file=FileHeader,exist=fexist)
          if(fexist) then
             if (ihr < 0) ihr1=i
             ihr=i
          end if
       ...
       if(ihr<0) then
          WhatPrint='ERROR: No input file available for regional (BRAMS) analysis. Program stops'
          write(6,'(2(1x,A))')trim(myname_),trim(WhatPrint)
          call stop2(99)
       end if
       ...
       write(FileHeader,'("BRAMS.head.",I2.2)') ihrmid
       write(FileFct,'("BRAMS.vfm.",I2.2)') ihrmid

       iret = brams%BrOpen(FileHeader, FileFct)
       if (iret .ne. 0)then
       ...
       regional_time(1) = brams%GetTimeInfo('fyr') ! year
       regional_time(2) = brams%GetTimeInfo('fmo') ! month
       regional_time(3) = brams%GetTimeInfo('fdy') ! day
       regional_time(4) = brams%GetTimeInfo('fhr') ! hour
       regional_time(5) = brams%GetTimeInfo('fmn') ! minute
       regional_time(6) = brams%GetTimeInfo('fsc') ! second
       regional_fhr     = zero  !that is not available/nor seems currently necessary
       nlon_regional    = brams%GetDim('nnxp') ! at native grid
       nlat_regional    = brams%GetDim('nnyp') ! at native grid
       nlon             = nlon_regional        ! at native grid
       nlat             = nlat_regional        ! at native grid
       nsig             = brams%GetDim('nnzp')

       if(diagnostic_reg.and.mype==0) then

          WhatPrint='yr,mn,dy,h,m,s='
          write(6,'(2(1x,A),6I6)')trim(myname_),trim(WhatPrint),regional_time

          WhatPrint='nlon_regional'
          write(6,'(2(1x,A),I6)')trim(myname_),trim(WhatPrint),nlon_regional

          WhatPrint='nlat_regional'
          write(6,'(2(1x,A),I6)')trim(myname_),trim(WhatPrint),nlat_regional

          WhatPrint='nsig'
          write(6,'(2(1x,A),I6)')trim(myname_),trim(WhatPrint),nsig

       endif

       !
       ! get geographic information about BRAMS
       !

!
!

!       info at a native grid
       allocate(glat(nlon,nlat))
       call brams%GetMapCoord('glat', glat)

       allocate(glon(nlon,nlat))
       call brams%GetMapCoord('glon', glon)

       allocate(dy(nLon,nLat))
       call brams%GetMapCoord('dxt', dy)

       allocate(dx(nLon,nLat))
       call brams%GetMapCoord('dyt', dx)

!      Lat/lon info should be in radians
       allocate(glat_an(nlon,nlat))
       allocate(glon_an(nlon,nlat))

       glon_an = real(glon,r_kind)*deg2rad
       glat_an = real(glat,r_kind)*deg2rad

       deallocate(glon)
       deallocate(glat)

       allocate(region_lat(nlat,nlon))
       allocate(region_lon(nlat,nlon))
       allocate(region_dy(nlat,nlon))
       allocate(region_dx(nlat,nlon))
       allocate(region_dyi(nlat,nlon))
       allocate(region_dxi(nlat,nlon))
       allocate(coeffy(nlat,nlon))
       allocate(coeffx(nlat,nlon))

       do i=1,nlon
          do j=1,nlat

             region_lon(j,i) = glon_an(i,j)
             region_lat(j,i) = glat_an(i,j)
             region_dx(j,i)  = dx(i,j)
             region_dy(j,i)  = dy(i,j)
             region_dxi(j,i) = one/dx(i,j)
             region_dyi(j,i) = one/dy(i,j)

             coeffx(j,i)     = half/region_dx(j,i)
             coeffy(j,i)     = half/region_dy(j,i)
          enddo
       enddo
       ...
       if(diagnostic_reg.and.mype==0) then
          ...
          WhatPrint='nlon,nlat='
          write(6,'(2(1x,A),2I8.4)')trim(myname_),trim(WhatPrint),nlon,nlat

          WhatPrint='rlat_min_dd='
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),rlat_min_dd
          WhatPrint='rlat_max_dd='
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),rlat_max_dd
          WhatPrint='rlon_min_dd='
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),rlon_min_dd
          WhatPrint='rlon_max_dd='
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),rlon_max_dd

          WhatPrint='rlat_min_ll='
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),rlat_min_ll
          WhatPrint='rlat_max_ll='
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),rlat_max_ll
          WhatPrint='rlon_min_ll='
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),rlon_min_ll
          WhatPrint='rlon_max_ll='
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),rlon_max_ll
       ...
       !
       ! Get info to make mean atmospheric profile
       !

       VarNLevs = brams%GetVarNLevels('PI', iret)
       allocate(grid(nlon,nlat))
       allocate(bramsPrslAvg(VarNLevs))
       do i=1,VarNLevs
          call brams%GetField(grid,'PI', level=i)
          grid = (grid/cp)**cpor*p00*0.01_r_kind 
          bramsPrslAvg(i) = (sum(grid)/(nLon*nLat)) ! ( hPa )
       enddo

       call brams%GetField(grid, 'SFC_PRESS', level=1)
       bramsPsfcAvg = sum(grid)/(nLon*nLat)  ! ( hPa )


       if(diagnostic_reg.and.mype==0) then
          WhatPrint='Averaged surface pressure (hPa) = '
          write(6,'(2(1x,A),F9.3)')trim(myname_),trim(WhatPrint),bramsPsfcAvg

          WhatPrint='Averaged pressure levels (hPa):'
          write(6,'(2(1x,A))')trim(myname_),trim(WhatPrint)
          do i=1,VarNLevs
             write(6,'(I4,F9.3)')i,bramsPrslAvg(i)
          enddo
       endif

       deallocate(grid)
       deallocate(glon_an, glat_an)

       allocate(RTGT(nlat,nlon))
       allocate(topt(nlon,nlat))
       call brams%GetField(topt, 'TOPT', level=1)
       ztop = brams%GetMapInfo('ztop')
       do j=1,nlon
          do i=1,nlat
             RTGT(i,j) = 1.0 - topt(j,i)/ztop
          enddo
       enddo
       deallocate(topt)

       allocate(dzt(nsig))
       call brams%GetVerticalInfo('dzt', DZT) 

...
while ( substr( $ARGV[0], 0, 1 ) eq "-" )
 {
  ...
  if ( substr( $ARGV[0], 1, 7 ) eq "usewrf=" )
  {
    $sw_use_wrf = substr( $ARGV[0], 8 ) ;
  }
  ...
  if ( substr( $ARGV[0], 1, 5 ) eq "comp=" )
  {
    $sw_comp = substr( $ARGV[0], 6 ) ;
  }
  shift @ARGV ;
 }
  ...
  while ( <CONFIGURE_DEFAULTS> )
  {
    for $paropt ( @platforms ) 
    {
      if ( substr( $_, 0, 5 ) eq "#ARCH" && ( index( $_, $sw_os ) >= 0 ) && ( index( $_, $sw_mach ) >= 0 )
	   && ( index($_, $paropt) >= 0 ) && (index($_,$sw_comp) >= 0) )
      {
...
if ($sw_use_wrf == 1){
   open ARCH_POSTAMBLE, "< arch/postamble.wrf" or die "cannot open arch/postamble.wrf" ;
   $HWRF = $ENV{HWRF};
   if ($HWRF) {

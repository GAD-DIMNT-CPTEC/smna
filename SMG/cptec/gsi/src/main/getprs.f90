...
!$$$ end documentation block
  ...
  use constants,only: zero,half,one_tenth,rd_over_cp,one, two, grav, rd
  ...
  use gridmod,only: wrf_nmm_regional,nems_nmmb_regional,eta1_ll,eta2_ll,pdtop_ll,pt_ll,&
       regional,wrf_mass_regional,cmaq_regional,twodvar_regional, isBrams, rtgt, dzt
...
! Declare local variables
  ...
  real(r_kind) :: aux1, aux2
  real(r_kind) :: alpha, pstar
  ...
  if (regional) then
     ...
     elseif (isBrams)then
        ...
        do k=1,nsig
           do j=1,lon2
              do i=1,lat2
                 aux1  = (-1) * grav * rtgt(i,j)
                 aux2  = rd * ges_tv_it(i,j,k) * dzt(k)
                 alpha = aux1/aux2
                 pstar = ps(i,j)*exp(alpha)
                 prs(i,j,k) = 0.5 * (pstar + ps(i,j))
              enddo
           enddo
        enddo
     endif

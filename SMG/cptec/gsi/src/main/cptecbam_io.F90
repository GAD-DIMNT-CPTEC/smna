...
!$omp end parallel do
      ...
      call GenReload_uv ( guW,   & ! Input U component
                          gvW,   & !       V component
                          kmax,  & !       only kmax Pe process u and v
...
subroutine GenReload_uv( UGrdIn, VGrdIn, kmax, UGrdOut, VGrdOut)
...
! !INPUT PARAMETERS:
  ...
  real(r_kind),    intent(in) :: UGrdIn(:)
  real(r_kind),    intent(in) :: VGrdIn(:)
  integer(i_kind), intent(in) :: kmax
...
!BOC
  ...
  allocate (sub(grd%lat2*grd%lon2,npe))

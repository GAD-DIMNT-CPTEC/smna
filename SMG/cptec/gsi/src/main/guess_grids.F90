...
! !USES:
    use mpimod, only: mpi_rtype,mpi_comm_world, npe, mpi_sum
    use constants,only: zero,one,rd_over_cp,one_tenth,half,ten, two, r100, cpor, cp
...
!   Declare local variables
    ...
    real(r_kind),dimension(:,:,:),pointer::ges_pi=>NULL()

    integer(i_kind) i,j,k,jj,itv,ips
    integer(i_kind) ierr
    logical ihaveprs(nfldsig)
    real :: profile(nsig), psavg
    real(r_kind) :: press(nsig)
    ...
    do jj=1,nfldsig
       ...
       if(itv/=0) call die(myname_,': tv not available in guess, abort',itv)

       if (regional .and. isBrams)then
          call gsi_bundlegetpointer(gsi_metguess_bundle(jj),'pi' ,ges_pi,ips)
          if(ips/=0) call die(myname_,': pi not available in guess, abort',ips)
       endif
          ...
          do j=1,lon2
             do i=1,lat2
                if(regional) then
                   ...
                   if(isBrams)then
                   ! brams -- Esta na face da camada (face superior)
                   !--------------------------------------------- k = 3 - meio da camada
                   !                                          face superior da camada 2 e inferior da camada 3
                   !--------------------------------------------- k = 2 - meio da camada
                   ! ///////////////////////////////////// < - superficie da terra -------- face inferior da camada 2
                   ! -------------------------------------------- k = 1 -meio da camada
                      ! kPa
                      if (k<=nsig) press(k) = (((ten*ges_pi(i,j,k))/cp)**cpor)*r100 ! kPa

                      if(k == 1)then
                         ges_prsi(i,j,k,jj) = ges_ps(i,j) ! kPa
                      else if(k == nsig)then
                         ges_prsi(i,j,k,jj) = press(k-1) - two * (press(k-2) - press(k-1))
                      else if(k == nsig+1)then
                         ges_prsi(i,j,k,jj) = ges_prsi(i,j,k-1,jj) - two * (ges_prsi(i,j,k-2,jj) - ges_prsi(i,j,k-1,jj))
                      else     
                         ! E a pressao na face superior da camada k
                         ges_prsi(i,j,k,jj) = half * (press(k)+press(k+1))
                      endif
       ...
       if(isBrams)then
          do jj = 1, nfldsig
             do k = 1, nsig
                do j = 1, lon2
                   do i = 1, lat2
                      ges_prsl(i,j,k,jj) = ((ten*ges_pi(i,j,k))/cp)**cpor*r100 ! (kPa)
                      ges_lnprsl(i,j,k,jj) = log(ges_prsl(i,j,k,jj))
                   enddo
                enddo
              enddo
           enddo
       endif ! end if isBrams regional block
...
! surface pressure and pressure profile at the layer midpoints
    if (regional) then
       ...
       else if (isBrams)then
          ges_psfcavg = bramsPsfcAvg
          do k=1,nsig
             ges_prslavg(k) = bramsPrslAvg(k)
          enddo
       else
          do k=1,nsig
...
! !USES:
    ...
    use constants, only: one,rd_over_cp_mass,r1000,ten,zero,two,cp,cpor
...
!   Compute geopotential height at midpoint of each layer
    do jj=1,nfldsig
       ier=0
       if(isBrams)then
          call gsi_bundlegetpointer(gsi_metguess_bundle(jj) ,'pi' ,ges_pi ,istatus)
          ier=ier+istatus
       else
       ...
       do j=1,lon2
          do i=1,lat2
             ...
             do k=1,nsig
                ...
                if(isBrams)then
                   pbk(k) = ((ten*ges_pi(i,j,k))/cp)**cpor*r1000 !hPa
                endif
...
!   Get time interpolation factors for sigma files
    ...
    ier=ier+istatus
    call gsi_bundlegetpointer(gsi_metguess_bundle(itsig) ,'ps' ,ges_ps_itsig ,istatus) ! bug fix
    ier=ier+istatus                                                                    ! bug fix
    call gsi_bundlegetpointer(gsi_metguess_bundle(itsigp),'ps' ,ges_ps_itsigp,istatus) ! bug fix
    ier=ier+istatus                                                                    ! bug fix
    if(ier/=0) return

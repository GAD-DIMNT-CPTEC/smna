...
!--------------------------------------------------------------------------!
! JGERD - A T E N T I O N
! JGERD the CONVERT in open statment dont work for cce 8.1 used at cptec
!       at this moment this routine isnt used at cptec, so to
!       compile we remove convert !!!! for new cce compiler  the
!       convert statment work fine and should be used here!
#ifdef CCE
   ! we compile with -h byteswapio. XE6 is little-endian, but all files are
   ! big-endian, except this file that is little-endian
   call asnfile(trim(fname),'-N swap_endian')

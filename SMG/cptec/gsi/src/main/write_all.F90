...
! !USES:
  ...
  use gridmod, only: regional, use_gfs_nemsio, center_name
  ...
  use cptecbam_io, only: write_bam
  use cptecbrams_io, only: writeBrams
...
! End of global block
  ...
  case ('CPTEC')
     if(.not.regional)then
        call write_bam(increment,mype,mype_atm,mype_sfc)
     else
        call writeBrams(increment,mype,mype_atm,mype_sfc)
     endif


  case default
     call die('WRITE_ALL',': Center not yet implemented', center)

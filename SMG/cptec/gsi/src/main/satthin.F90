...
!$$$
    ...
    use cptecbam_io, only: read_bamsfc
    use cptecbrams_io, only: readBramsSfc
...
200             format('sfcf',i2.2)
          ...
          select case (trim(center))
             ...
             case('CPTEC')
                ...
                call read_bamsfc( mype_io, mype,  &
                                  fact10_full,    & ! 10-meter wind factor
                                  sst_full,       & ! surface temperature [k]
                                  sno_full,       & ! snow depth [m]
                                  veg_type_full,  & ! vegetation type  [1-13]
                                  veg_frac_full,  & ! vegetation cover [-]
                                  soil_type_full, & ! soil type [1-9]
                                  soil_temp_full, & ! surface soil temperature [k]
                                  soil_moi_full,  & ! surface soil moisture [fraction]
                                  isli_full,      & ! land sea mask 
                                  sfc_rough_full, & ! surface roughness length [cm]   
                                  zs_full_gfs,    & ! Orography [ m ]
                                  use_sfc_any)
          end select

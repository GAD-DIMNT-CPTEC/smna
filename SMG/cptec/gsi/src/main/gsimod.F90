...
! !USES:
  ...
  use gridmod, only: nlat,nlon,nsig,wrf_nmm_regional,nems_nmmb_regional,cmaq_regional,&
     ...
     jcap_gfs,nlat_gfs,nlon_gfs,jcap_cut, center_name, isBrams
...
!     center_name - Center name that come from First Guess
...
!            add use of guess file later for regional mode.
  ...
  namelist/setup/center_name, gencode,factqmin,factqmax,clip_supersaturation, &
...
!     IsBRAMS           - logical for input from BRAMS regional model
...
!                         poles.  if .false., then gaussian grid assumed for ensemble (global only)
  ...
  namelist/gridopts/jcap,jcap_b,nsig,nlat,nlon,nlat_regional,nlon_regional,&
       diagnostic_reg,update_regsfc,netcdf,regional,IsBRAMS, wrf_nmm_regional,nems_nmmb_regional,&
...
! Set regional parameters
  ...
  regional=wrf_nmm_regional.or.wrf_mass_regional.or.twodvar_regional.or.nems_nmmb_regional .or. cmaq_regional .or. IsBRAMS
...
! Initialize constants
  call init_constants(regional, isBrams)
  call gps_constants(use_compress)

! If this is a wrf regional run, then run interface with wrf
  update_pint=.false.
  if (regional .and. .not. IsBRAMS) call convert_regional_guess(mype,ctph0,stph0,tlm0) ! Not necessary for BRAMS
